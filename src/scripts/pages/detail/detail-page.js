...
export default class DetailPage {
    ...
    return `
      <section class="px-8 py-8 max-w-4xl mx-auto">
        <div id="detail-container" class="bg-white p-6 rounded-lg shadow-md">
          <p class="text-[#af69ee]">Loading detail cerita...</p>
        </div>
      </section>
    `;
  }
  ...
  renderStoryDetail(story) {
    ...
    document.getElementById("detail-container").innerHTML = `
      <h2 class="text-2xl font-bold mb-4 text-[#af69ee]">${story.name}</h2>
      <img src="${
        story.photoUrl || "https://via.placeholder.com/600x400"
      }" alt="Foto Cerita" class="w-full rounded-lg mb-4 object-cover max-h-96" />
      <p class="mb-4">${story.description}</p>
      <p><strong class="text-[#af69ee]">Lokasi:</strong> ${
        story.lat && story.lon
