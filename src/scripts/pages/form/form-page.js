...
class FormPage {
  async render() {
    return `
      <section class="px-6 py-10 max-w-2xl mx-auto">
        <h1 class="text-3xl font-extrabold text-center text-[#af69ee] mb-8 tracking-tight">Tambah Cerita Baru</h1>
        <form id="formStory" class="space-y-6 bg-white p-6 rounded-xl shadow-lg border border-[#e6ccfa]">
          <div>
            <label for="description" class="block text-sm font-semibold mb-1 text-gray-700">
              <i class="fas fa-align-left mr-2 text-[#af69ee]"></i>Deskripsi Cerita
            </label>
            <textarea id="description" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#af69ee]" rows="4" required aria-label="Deskripsi Cerita"></textarea>
          </div>
          <div>
            <label class="block text-sm font-semibold mb-2 text-gray-700">
              <i class="fas fa-camera-retro mr-2 text-[#af69ee]"></i>Pilih Foto Cerita
            </label>
            <div class="flex gap-4">
              <button type="button" id="cameraButton" class="flex-1 bg-[#af69ee] text-white py-2 rounded-md hover:bg-[#9b54d6] transition text-sm" aria-label="Ambil foto menggunakan kamera">
            ...
            <button type="button" id="captureButton" class="w-full mt-4 bg-[#af69ee] text-white py-2 rounded-md hover:bg-[#9b54d6] transition hidden">
          ...
          <div id="mapContainer">
            <label class="block text-sm font-semibold mb-1 text-gray-700">
              <i class="fas fa-map-marker-alt mr-2 text-[#af69ee]"></i> Lokasi Cerita
            </label>
          ...
          <button type="submit" class="w-full bg-[#af69ee] text-white py-3 rounded-lg hover:bg-[#9b54d6] font-semibold tracking-wide transition">
  ...
  async afterRender() {
    ...
    let marker = L.marker([selectedLat, selectedLon], {
      draggable: true,
    }).addTo(map);
    ...
    map.on("click", function (e) {
      ...
      selectedLon = lng;
      marker
        .bindPopup(`Lokasi: Latitude: ${lat}, Longitude: ${lng}`)
        .openPopup();
    });
  }
