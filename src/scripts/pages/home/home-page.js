  ...
  async render() {
    return `
      <section class="px-8 py-8 max-w-6xl mx-auto">
        <h1 class="text-3xl font-bold text-center text-[#af69ee] mb-6">Daftar Cerita</h1>
        <div class="mb-6">
          <input 
            id="searchInput" 
            type="text" 
            placeholder="Cari cerita..." 
            class="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#af69ee]"
          />
        </div>
        ...
        <div id="pagination" class="flex justify-between mt-6 mb-10">
          <button id="prevButton" class="bg-[#af69ee] text-white px-4 py-2 rounded-md hover:bg-[#9f50e6] disabled:bg-gray-300" disabled>
            Sebelumnya
          </button>
          <button id="nextButton" class="bg-[#af69ee] text-white px-4 py-2 rounded-md hover:bg-[#9f50e6] disabled:bg-gray-300">
            Selanjutnya
          </button>
        </div>
        <div>
          <h3 class="text-2xl font-semibold text-[#af69ee] mb-4">Peta Lokasi Cerita</h3>
  ...
  async afterRender() {
    ...
    const displayStories = async (stories) => {
      ...
      for (const story of stories) {
        ...
        const location =
          story.lat && story.lon
            ? await HomePresenter.getLocationName(story.lat, story.lon)
        ...
        storyElement.innerHTML = `
          ...
          <div class="mt-4 flex space-x-4">
            <button class="saveButton text-yellow-600 hover:text-yellow-800" data-id="${
              story.id
            }">
    ...
    searchInput.addEventListener("input", async () => {
      ...
      const filteredStories = result.listStory.filter(
        (story) =>
    ...
    result.listStory.forEach((story) => {
      if (story.lat && story.lon) {
        ...
        marker.bindPopup(
          `<strong>${story.name}</strong><br>${story.description.slice(
            0,
            100
          )}...`
        );
